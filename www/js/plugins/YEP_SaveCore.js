...
Window_SaveInfo.prototype.drawInvalidText = function(dy) {
  ...
  if (this._info) {
	var text = TextManager.yanflySaveInvalidText;
    //var text = Yanfly.Param.SaveInfoInvalid;
  } else {
	var text = TextManager.yanflySaveEmpty;
...
Window_SaveInfo.prototype.drawFace = function(name, index, x, y, w, h) {
    ...
    if (bitmap.width <= 0) {
      return setTimeout(this.drawFace.bind(this, name, index, x + remXPadding, y, w, h), 50);
    }
...
Window_SaveInfo.prototype.drawPartyNames = function(dy) {
  ...
  */
	...
	if(this._saveContents.actors._data[ACTOR_KARRYN_ID]) {
		let titleText = '';
		
		titleText += this._saveContents.actors._data[ACTOR_KARRYN_ID].getTitleText();
		
		this.drawText(titleText, dx, dy, dw, 'center');
		dx += dw
	}
...
Window_SaveInfo.prototype.drawPartyLevels = function(dy) {
  ...
  */
	...
	if(this._saveContents.actors._data[ACTOR_KARRYN_ID]) {
		let wardenLv = this._saveContents.actors._data[ACTOR_KARRYN_ID].level;
		let slutLv = this._saveContents.actors._data[ACTOR_KARRYN_ID].slutLvl;
		
		let lvText = '' + TextManager.wardenLevel;
		if(TextManager.isEnglish) lvText += ' ';
		lvText += '' + wardenLv;
		
		if(DEBUG_MODE && slutLv > 0) {
...
Window_SaveInfo.prototype.drawColumn = function(column, dx, dy, dw) {
    ...
    for (let i = 0; i < length; ++i) {
      ...
      if (data.toUpperCase().trim() !== 'EMPTY') {
		let prologueEnded = this._saveContents.switches._data[SWITCH_PROLOGUE_ENDED] === true;
		if(data.toUpperCase().trim() !== 'PLAYTIME' && !prologueEnded) {
			dy += this.lineHeight();
			continue;
		}
		if((data.toUpperCase().trim() === 'TOTAL DAYS' && !this._saveContents.party._totalPlaythroughs) ||
		(data.toUpperCase().trim() === 'TOTAL PLAYTHROUGHS' && !this._saveContents.party._totalPlaythroughs) ||
		(data.toUpperCase().trim() === 'ENDINGS' && !this._saveContents.party._endingsSeenCount)) {
			dy += this.lineHeight();
			continue;
		}
...
Window_SaveInfo.prototype.drawData = function(data, dx, dy, dw) {
  if (data.toUpperCase().trim() === 'NULL') {
	...
	} else if (data.toUpperCase().trim() === 'PRISON LEVEL') {
    this.drawPrisonLevel(dx, dy, dw);
	} else if (data.toUpperCase().trim() === 'DIFFICULTY') {
	...
	} else if (data.toUpperCase().trim() === 'INCOME') {
    this.drawIncome(dx, dy, dw);
	} else if (data.toUpperCase().trim() === 'EXPENSE') {
    this.drawExpense(dx, dy, dw);
	} else if (data.toUpperCase().trim() === 'DATE') {
    this.drawDate(dx, dy, dw);
	} else if (data.toUpperCase().trim() === 'ORDER') {
    this.drawOrder(dx, dy, dw);
	} else if (data.toUpperCase().trim() === 'CONTROL') {
    this.drawControl(dx, dy, dw);
	} else if (data.toUpperCase().trim() === 'CORRUPTION') {
    this.drawCorruption(dx, dy, dw);
	} else if (data.toUpperCase().trim() === 'TITLE COUNT') {
    this.drawTitleCount(dx, dy, dw);
	} else if (data.toUpperCase().trim() === 'SLUT LEVEL') {
    this.drawSlutLevel(dx, dy, dw);
	} else if (data.toUpperCase().trim() === 'TOTAL DAYS') {
    this.drawTotalDays(dx, dy, dw);
	} else if (data.toUpperCase().trim() === 'TOTAL PLAYTHROUGHS') {
    this.drawTotalPlaythroughs(dx, dy, dw);
	} else if (data.toUpperCase().trim() === 'ENDINGS') {
...
Window_SaveInfo.prototype.drawLocation = function(dx, dy, dw) {
    ...
    let id = this._saveContents.map._mapId;
    if (Yanfly.Param.SaveMapDisplayName) {
		var text = '';
		if(TextManager.isJapanese) {
			if(!this._saveContents.map.locationDisplayNameRem) text = '';
			else text = this._saveContents.map.locationDisplayNameRem[0] || '';
		}
		else if(TextManager.isEnglish) {
			if(!this._saveContents.map.locationDisplayNameRem) text = '';
			else text = this._saveContents.map.locationDisplayNameRem[1] || '';
		}
		else text = this._saveContents.map.locationDisplayName || '';
...
Window_SaveInfo.prototype.drawPrisonLevel = function(dx, dy, dw) {
    ...
    let levelName = this._saveContents.party.getCurrentPrisonLevelName();
	this.changeTextColor(this.systemColor());
	this.drawText(levelName, dx, dy, dw, 'center');
	this.resetTextColor();
};

Window_SaveInfo.prototype.drawDifficulty = function(dx, dy, dw) {
    let difficulty = TextManager.SecretaryMode;
	if(this._saveContents.party.normalMode()) {
		this.changeTextColor(this.textColor(2));
		difficulty = TextManager.WardenMode;
	}
	else if(this._saveContents.party.hardMode()) {
		this.changeTextColor(this.textColor(18));
		difficulty = TextManager.PrisonerMode;
	}
	this.drawText(difficulty, dx, dy, dw, 'center');
	this.resetTextColor();
};
...
Window_SaveInfo.prototype.drawGoldCount = function(dx, dy, dw) {
    ...
    let text = Yanfly.Util.toGroup(this._saveContents.party._gold);
...
Window_SaveInfo.prototype.drawIncome = function(dx, dy, dw) {
    ...
    this.changeTextColor(this.normalColor());  
    this.drawText('+' + this._saveContents.party._savedIncomeValue, dx, dy, dw, 'right');
};
Window_SaveInfo.prototype.drawExpense = function(dx, dy, dw) {
    ...
    this.changeTextColor(this.normalColor());  
    this.drawText('-' + this._saveContents.party._savedExpenseValue, dx, dy, dw, 'right');
};
Window_SaveInfo.prototype.drawOrder = function(dx, dy, dw) {
    ...
    this.changeTextColor(this.normalColor());  
    this.drawText(this._saveContents.party.order, dx, dy, dw, 'right');
};
...
Window_SaveInfo.prototype.drawCorruption = function(dx, dy, dw) {
    ...
    this.changeTextColor(this.normalColor());  
    this.drawText(this._saveContents.party.corruption, dx, dy, dw, 'right');
};
...
Window_SaveInfo.prototype.drawTitleCount = function(dx, dy, dw) {
	...
	let titleCount = this._saveContents.actors._data[ACTOR_KARRYN_ID].getTotalTitlesCount();
	
    this.changeTextColor(this.systemColor());
    this.drawText(TextManager.titles + ":", dx, dy, dw, 'left');
    this.changeTextColor(this.normalColor());  
    this.drawText(titleCount, dx, dy, dw, 'right');
};

Window_SaveInfo.prototype.drawSlutLevel = function(dx, dy, dw) {
	if(!DEBUG_MODE || this._saveContents.actors._data[ACTOR_KARRYN_ID].slutLvl == 0) return;
    this.changeTextColor(this.systemColor());
    this.drawText(TextManager.slutLevel + ":", dx, dy, dw, 'left');
    this.changeTextColor(this.normalColor());  
    this.drawText(this._saveContents.actors._data[ACTOR_KARRYN_ID].slutLvl, dx, dy, dw, 'right');
};
...
Window_SaveInfo.prototype.drawDate = function(dx, dy, dw) {
    ...
    this.changeTextColor(this.normalColor());  
    this.drawText(this._saveContents.party.date, dx, dy, dw, 'right');
};
Window_SaveInfo.prototype.drawTotalDays = function(dx, dy, dw) {
	if(!this._saveContents.party._totalPlaythroughs) return;
    this.changeTextColor(this.systemColor());
    this.drawText(TextManager.yanflySaveTotalDays, dx, dy, dw, 'left');
    this.changeTextColor(this.normalColor());  
    this.drawText(this._saveContents.party._dayCount, dx, dy, dw, 'right');
};
Window_SaveInfo.prototype.drawTotalPlaythroughs = function(dx, dy, dw) {
    ...
    this.changeTextColor(this.normalColor());  
    this.drawText(this._saveContents.party._totalPlaythroughs, dx, dy, dw, 'right');
};
Window_SaveInfo.prototype.drawEndings = function(dx, dy, dw) {
	if(!this._saveContents.party._endingsSeenCount) return;
    this.changeTextColor(this.systemColor());
    this.drawText(TextManager.yanflySaveTotalEndings, dx, dy, dw, 'left');
    this.changeTextColor(this.normalColor());  
    this.drawText(this._saveContents.party._endingsSeenCount, dx, dy, dw, 'right');
};
