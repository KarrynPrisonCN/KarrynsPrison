...
const REM_BHD_BC_LAY_X = 0;
const REM_BHD_BC_LAY_Y = 0;

const REM_BHD_BC_COM_X = 35;
const REM_BHD_BC_COM_Y = 44;
const REM_BHD_BC_RANGE = 93;

const REM_BHD_BC_NAME_X = 130;
const REM_BHD_BC_NAME_Y = 130;
const REM_BHD_BC_NAME_FONT_SIZE = 22;
...
const REM_BHD_BC_DEFEATED_ONE_X = -530;
const REM_BHD_BC_DEFEATED_ONE_Y = 45;

const REM_BHD_BC_DEFEATED_TWO_X = 585;
const REM_BHD_BC_DEFEATED_TWO_Y = -210;

const REM_BHD_BC_DEFEATED_THREE_X = -510;
const REM_BHD_BC_DEFEATED_THREE_Y = 20;
...
const REM_BHD_BC_WAITRESS_SERVE_X = 210;
const REM_BHD_BC_WAITRESS_SERVE_Y = 40;
...
const REM_BHD_BC_ORC_PAIZURI_X = -98;
const REM_BHD_BC_ORC_PAIZURI_Y = 70;

const REM_BHD_BC_GUARD_GANGBANG_X = 125;
const REM_BHD_BC_GUARD_GANGBANG_Y = -325;

const REM_BHD_BC_COMBAT_X = -95;
const REM_BHD_BC_COMBAT_Y = -60;

const REM_BHD_BC_UNARMED_X = -110;
const REM_BHD_BC_UNARMED_Y = 65;
...
const REM_BHD_BC_BJ_KNEELING_X = -95;
const REM_BHD_BC_BJ_KNEELING_Y = 60;
...
const REM_BHD_BC_KARRYN_COWGIRL_X = 575;
...
Game_Interpreter.prototype.command129 = function() {	
	...
	$gameTemp._pcom_need_refresh = true;
	return true;
};
...
Window_ActorCommand.prototype.com_pic_initialize = function() {
	...
	this._rol_range = REM_BHD_BC_RANGE;
...
Window_ActorCommand.prototype.load_com_images = function() {
	...
	for (var i = 0; i < this._list.length; i++) {
		 ...
		 for (var r = 0; r < this._list.length; r++) {
			...
			//	this._com_images.push(ImageManager.loadBcom("Com_Attack"));
			//
			//else if(this._list[r].symbol == "equip change") 
			//	this._com_images.push(ImageManager.loadBcom("Com_Equip"));
			//
			if(this._list[r].symbol == "guard") {
				this._com_images.push(ImageManager.loadBcom("Com_Rest"));
			}
			else if(this._list[r].symbol == "escape") {
				this._com_images.push(ImageManager.loadBcom("Com_Escape"));
			}
			else if(this._list[r].symbol == "battleLog") {
				this._com_images.push(ImageManager.loadBcom("Com_BattleLog"));
			}
			else if(this._list[r].symbol == "status") {
				this._com_images.push(ImageManager.loadBcom("Com_Status"));
			}
		    //else this._com_images.push(ImageManager.loadBcom("Com_" + this._list[r].name));
			else this._com_images.push(ImageManager.loadBcom("Com_" + this._list[r].ext));
		 };
	};
...
Window_ActorCommand.prototype.create_layout = function() {	
	...
	this._layout.x = REM_BHD_BC_LAY_X;
	this._layout.y = REM_BHD_BC_LAY_Y;
	if(Karryn.isInUnarmedPose()) {
		this._layout.x += REM_BHD_BC_UNARMED_X;
		this._layout.y += REM_BHD_BC_UNARMED_Y;
	}
	else if(Karryn.isInCombatPose()) {
		this._layout.x += REM_BHD_BC_COMBAT_X;
		this._layout.y += REM_BHD_BC_COMBAT_Y;
	}
	else if(Karryn.isInDownFallDownPose()) {
		this._layout.x += REM_BHD_BC_DOWN_FALL_X;
		this._layout.y += REM_BHD_BC_DOWN_FALL_Y;
	}
	else if(Karryn.isInDownOrgasmPose()) {
		this._layout.x += REM_BHD_BC_DOWN_ORG_X;
		this._layout.y += REM_BHD_BC_DOWN_ORG_Y;
	}
	else if(Karryn.isInDownStaminaPose()) {
		...
		this._layout.y += REM_BHD_BC_DEFEATED_ONE_Y;
	}
		...
		this._layout.x += REM_BHD_BC_WAITRESS_SERVE_X;
		this._layout.y += REM_BHD_BC_WAITRESS_SERVE_Y;
	}
	else if(Karryn.isInWaitressSexPose()) {
		this._layout.x += REM_BHD_BC_WAITRESS_SEX_X;
		this._layout.y += REM_BHD_BC_WAITRESS_SEX_Y;
	}
	else if(Karryn.isInThugGangbangPose()) {
		this._layout.x += REM_BHD_BC_THUG_GANGBANG_SEX_X;
		this._layout.y += REM_BHD_BC_THUG_GANGBANG_SEX_Y;
	}
	else if(Karryn.isInGuardGangbangPose()) {
		this._layout.x += REM_BHD_BC_GUARD_GANGBANG_X;
		this._layout.y += REM_BHD_BC_GUARD_GANGBANG_Y;
	}
	...
	else if(Karryn.isInStandingHJSexPose()) {
		this._layout.x += REM_BHD_BC_HJ_STANDING_X;
		this._layout.y += REM_BHD_BC_HJ_STANDING_Y;
	}
	else if(Karryn.isInLayingTittyfuckSexPose()) {
		this._layout.x += REM_BHD_BC_PAIZURI_LAYING_X;
		this._layout.y += REM_BHD_BC_PAIZURI_LAYING_Y;
	}
	else if(Karryn.isInSlimeAnalPiledriverSexPose()) {
		this._layout.x += REM_BHD_BC_SLIME_PILEDRIVER_X;
		this._layout.y += REM_BHD_BC_SLIME_PILEDRIVER_Y;
	}
	else if(Karryn.isInReceptionistPose()) {
		this._layout.x += REM_BHD_BC_RECEPTIONIST_X;
		this._layout.y += REM_BHD_BC_RECEPTIONIST_Y;
	}
	else if(Karryn.isInOrcPaizuriSexPose()) {
		this._layout.x += REM_BHD_BC_ORC_PAIZURI_X;
		this._layout.y += REM_BHD_BC_ORC_PAIZURI_Y;
	}
	else if(Karryn.isInGoblinCunnilingusSexPose()) {
		this._layout.x += REM_BHD_BC_GOBLIN_CL_X;
		this._layout.y += REM_BHD_BC_GOBLIN_CL_Y;
	}
	else if(Karryn.isInReverseCowgirlPose()) {
		this._layout.x += REM_BHD_BC_REVERSE_COWGIRL_X;
		this._layout.y += REM_BHD_BC_REVERSE_COWGIRL_Y;
	}
	else if(Karryn.isInKarrynCowgirlPose()) {
		this._layout.x += REM_BHD_BC_KARRYN_COWGIRL_X;
		this._layout.y += REM_BHD_BC_KARRYN_COWGIRL_Y;
	}
...
Window_ActorCommand.prototype.create_commands = function() {	
	for (var i = 0; i < this._max_com; i++) {
		 ...
		 // if(i == 3 || (i >= 15) && (i <=16)) this._com_sprites[i].enabled = true;
...
Window_ActorCommand.prototype.create_com_name = function() {	
	...
	this._com_name = new Sprite(new Bitmap(REM_BHD_BC_NAME_WIDTH,REM_BHD_BC_NAME_HEIGHT))
	this._com_name.x = REM_BHD_BC_NAME_X;
	this._com_name.y = REM_BHD_BC_NAME_Y; 
	if(Karryn.isInUnarmedPose()) {
		this._com_name.x += REM_BHD_BC_UNARMED_X;
		this._com_name.y += REM_BHD_BC_UNARMED_Y; 
	}
	else if(Karryn.isInCombatPose()) {
		this._com_name.x += REM_BHD_BC_COMBAT_X;
		this._com_name.y += REM_BHD_BC_COMBAT_Y; 
	}
	else if(Karryn.isInDownFallDownPose()) {
		this._com_name.x += REM_BHD_BC_DOWN_FALL_X;
		this._com_name.y += REM_BHD_BC_DOWN_FALL_Y; 
	}
	else if(Karryn.isInDownOrgasmPose()) {
		this._com_name.x += REM_BHD_BC_DOWN_ORG_X;
		this._com_name.y += REM_BHD_BC_DOWN_ORG_Y; 
	}
	else if(Karryn.isInDownStaminaPose()) {
		...
		this._com_name.y += REM_BHD_BC_DEFEATED_ONE_Y; 
	}
	else if(Karryn.isInDefeatedLevel2Pose()) {
		...
		this._com_name.x += REM_BHD_BC_WAITRESS_SERVE_X;
		this._com_name.y += REM_BHD_BC_WAITRESS_SERVE_Y; 
	}
	else if(Karryn.isInWaitressSexPose()) {
		this._com_name.x += REM_BHD_BC_WAITRESS_SEX_X;
		this._com_name.y += REM_BHD_BC_WAITRESS_SEX_Y; 
	}
	else if(Karryn.isInThugGangbangPose()) {
		this._com_name.x += REM_BHD_BC_THUG_GANGBANG_SEX_X;
		this._com_name.y += REM_BHD_BC_THUG_GANGBANG_SEX_Y; 
	}
	else if(Karryn.isInGuardGangbangPose()) {
		this._com_name.x += REM_BHD_BC_GUARD_GANGBANG_X;
		this._com_name.y += REM_BHD_BC_GUARD_GANGBANG_Y; 
	}
	else if(Karryn.isInFootjobSexPose()) {
		...
		this._com_name.x += REM_BHD_BC_HJ_STANDING_X;
		this._com_name.y += REM_BHD_BC_HJ_STANDING_Y; 
	}
	else if(Karryn.isInLayingTittyfuckSexPose()) {
		this._com_name.x += REM_BHD_BC_PAIZURI_LAYING_X;
		this._com_name.y += REM_BHD_BC_PAIZURI_LAYING_Y; 
	}
	else if(Karryn.isInSlimeAnalPiledriverSexPose()) {
		this._com_name.x += REM_BHD_BC_SLIME_PILEDRIVER_X;
		this._com_name.y += REM_BHD_BC_SLIME_PILEDRIVER_Y; 
	}
	else if(Karryn.isInReceptionistPose()) {
		this._com_name.x += REM_BHD_BC_RECEPTIONIST_X;
		this._com_name.y += REM_BHD_BC_RECEPTIONIST_Y; 
	}
	else if(Karryn.isInOrcPaizuriSexPose()) {
		this._com_name.x += REM_BHD_BC_ORC_PAIZURI_X;
		this._com_name.y += REM_BHD_BC_ORC_PAIZURI_Y; 
	}
	else if(Karryn.isInGoblinCunnilingusSexPose()) {
		this._com_name.x += REM_BHD_BC_GOBLIN_CL_X;
		this._com_name.y += REM_BHD_BC_GOBLIN_CL_Y; 
	}
	else if(Karryn.isInReverseCowgirlPose()) {
		this._com_name.x += REM_BHD_BC_REVERSE_COWGIRL_X;
		this._com_name.y += REM_BHD_BC_REVERSE_COWGIRL_Y; 
	}
	else if(Karryn.isInKarrynCowgirlPose()) {
		this._com_name.x += REM_BHD_BC_KARRYN_COWGIRL_X;
		this._com_name.y += REM_BHD_BC_KARRYN_COWGIRL_Y; 
	}
	else if(Karryn.isInLizardmanCowgirlPose()) {
...
Window_ActorCommand.prototype.refresh_bitmap_com = function() {	
	...
	for (var i = 0; i < this.maxCom(); i++) {	
		this._com_sprites[i].x = this._wh[0] + REM_BHD_BC_COM_X;
		this._com_sprites[i].y = this._wh[1] + (this._wh[1] * i) + REM_BHD_BC_COM_Y;	
		if(Karryn.isInUnarmedPose()) {
			this._com_sprites[i].x += REM_BHD_BC_UNARMED_X;
			this._com_sprites[i].y += REM_BHD_BC_UNARMED_Y;	
		}
		else if(Karryn.isInCombatPose()) {
			this._com_sprites[i].x += REM_BHD_BC_COMBAT_X;
			this._com_sprites[i].y += REM_BHD_BC_COMBAT_Y;	
		}
		else if(Karryn.isInDownFallDownPose()) {
			this._com_sprites[i].x += REM_BHD_BC_DOWN_FALL_X;
			this._com_sprites[i].y += REM_BHD_BC_DOWN_FALL_Y;	
		}
		else if(Karryn.isInDownOrgasmPose()) {
			this._com_sprites[i].x += REM_BHD_BC_DOWN_ORG_X;
			this._com_sprites[i].y += REM_BHD_BC_DOWN_ORG_Y;	
		}
		else if(Karryn.isInDownStaminaPose()) {
			...
			this._com_sprites[i].y += REM_BHD_BC_DEFEATED_ONE_Y;	
		}
		else if(Karryn.isInDefeatedLevel2Pose()) {
			...
			this._com_sprites[i].x += REM_BHD_BC_WAITRESS_SERVE_X;
			this._com_sprites[i].y += REM_BHD_BC_WAITRESS_SERVE_Y;
		}
		else if(Karryn.isInWaitressSexPose()) {
			this._com_sprites[i].x += REM_BHD_BC_WAITRESS_SEX_X;
			this._com_sprites[i].y += REM_BHD_BC_WAITRESS_SEX_Y;
		}
		else if(Karryn.isInThugGangbangPose()) {
			this._com_sprites[i].x += REM_BHD_BC_THUG_GANGBANG_SEX_X;
			this._com_sprites[i].y += REM_BHD_BC_THUG_GANGBANG_SEX_Y;
		}
		else if(Karryn.isInGuardGangbangPose()) {
			this._com_sprites[i].x += REM_BHD_BC_GUARD_GANGBANG_X;
			this._com_sprites[i].y += REM_BHD_BC_GUARD_GANGBANG_Y;
		}
		else if(Karryn.isInFootjobSexPose()) {
			...
			this._com_sprites[i].x += REM_BHD_BC_HJ_STANDING_X;
			this._com_sprites[i].y += REM_BHD_BC_HJ_STANDING_Y;
		}
		else if(Karryn.isInLayingTittyfuckSexPose()) {
			this._com_sprites[i].x += REM_BHD_BC_PAIZURI_LAYING_X;
			this._com_sprites[i].y += REM_BHD_BC_PAIZURI_LAYING_Y;
		}
		else if(Karryn.isInSlimeAnalPiledriverSexPose()) {
			this._com_sprites[i].x += REM_BHD_BC_SLIME_PILEDRIVER_X;
			this._com_sprites[i].y += REM_BHD_BC_SLIME_PILEDRIVER_Y;
		}
		else if(Karryn.isInReceptionistPose()) {
			this._com_sprites[i].x += REM_BHD_BC_RECEPTIONIST_X;
			this._com_sprites[i].y += REM_BHD_BC_RECEPTIONIST_Y;
		}
		else if(Karryn.isInOrcPaizuriSexPose()) {
			this._com_sprites[i].x += REM_BHD_BC_ORC_PAIZURI_X;
			this._com_sprites[i].y += REM_BHD_BC_ORC_PAIZURI_Y;
		}
		else if(Karryn.isInGoblinCunnilingusSexPose()) {
			this._com_sprites[i].x += REM_BHD_BC_GOBLIN_CL_X;
			this._com_sprites[i].y += REM_BHD_BC_GOBLIN_CL_Y;
		}
		else if(Karryn.isInReverseCowgirlPose()) {
			this._com_sprites[i].x += REM_BHD_BC_REVERSE_COWGIRL_X;
			this._com_sprites[i].y += REM_BHD_BC_REVERSE_COWGIRL_Y;
		}
		else if(Karryn.isInKarrynCowgirlPose()) {
			this._com_sprites[i].x += REM_BHD_BC_KARRYN_COWGIRL_X;
			this._com_sprites[i].y += REM_BHD_BC_KARRYN_COWGIRL_Y;
		}
		else if(Karryn.isInLizardmanCowgirlPose()) {
...
Window_ActorCommand.prototype.refresh_ring_command = function() {
	...
	for (var i = 0; i < this.maxCom(); i++) {
    	...
    	this._com_sprites[i].x = this.width / 2 + REM_BHD_BC_COM_X;
    	this._com_sprites[i].y = this.height / 2 + REM_BHD_BC_COM_Y;	
		if(Karryn.isInUnarmedPose()) {
			this._com_sprites[i].x += REM_BHD_BC_UNARMED_X;
			this._com_sprites[i].y += REM_BHD_BC_UNARMED_Y;	
		}
...
Window_ActorCommand.prototype.update_position = function(i) {
	if (this._com_mode_b === 0) {
    	this._np[0] = this._wh[0] + REM_BHD_BC_COM_X + this._side[i][0];
	    this._np[1] = this._wh[1] + (this._wh[1] * i) - this.limit_height() + REM_BHD_BC_COM_Y + this._side[i][1];
		if(Karryn.isInUnarmedPose()) {
			this._np[0] += REM_BHD_BC_UNARMED_X;
			this._np[1] += REM_BHD_BC_UNARMED_Y;	
		}
		else if(Karryn.isInCombatPose()) {
			this._np[0] += REM_BHD_BC_COMBAT_X;
			this._np[1] += REM_BHD_BC_COMBAT_Y;	
		}
		else if(Karryn.isInDownFallDownPose()) {
			this._np[0] += REM_BHD_BC_DOWN_FALL_X;
			this._np[1] += REM_BHD_BC_DOWN_FALL_Y;	
		}
		else if(Karryn.isInDownOrgasmPose()) {
			this._np[0] += REM_BHD_BC_DOWN_ORG_X;
			this._np[1] += REM_BHD_BC_DOWN_ORG_Y;	
		}
		else if(Karryn.isInDownStaminaPose()) {
			...
			this._np[1] += REM_BHD_BC_DEFEATED_ONE_Y;	
		}	
		else if(Karryn.isInDefeatedLevel2Pose()) {
			...
			this._np[0] += REM_BHD_BC_WAITRESS_SERVE_X;
			this._np[1] += REM_BHD_BC_WAITRESS_SERVE_Y;	
		}
		else if(Karryn.isInWaitressSexPose()) {
			this._np[0] += REM_BHD_BC_WAITRESS_SEX_X;
			this._np[1] += REM_BHD_BC_WAITRESS_SEX_Y;	
		}
		else if(Karryn.isInThugGangbangPose()) {
			this._np[0] += REM_BHD_BC_THUG_GANGBANG_SEX_X;
			this._np[1] += REM_BHD_BC_THUG_GANGBANG_SEX_Y;	
		}
		else if(Karryn.isInGuardGangbangPose()) {
			this._np[0] += REM_BHD_BC_GUARD_GANGBANG_X;
			this._np[1] += REM_BHD_BC_GUARD_GANGBANG_Y;	
		}
		else if(Karryn.isInFootjobSexPose()) {
			...
			this._np[0] += REM_BHD_BC_HJ_STANDING_X;
			this._np[1] += REM_BHD_BC_HJ_STANDING_Y;	
		}
		else if(Karryn.isInLayingTittyfuckSexPose()) {
			this._np[0] += REM_BHD_BC_PAIZURI_LAYING_X;
			this._np[1] += REM_BHD_BC_PAIZURI_LAYING_Y;	
		}
		else if(Karryn.isInSlimeAnalPiledriverSexPose()) {
			this._np[0] += REM_BHD_BC_SLIME_PILEDRIVER_X;
			this._np[1] += REM_BHD_BC_SLIME_PILEDRIVER_Y;	
		}
		else if(Karryn.isInReceptionistPose()) {
			this._np[0] += REM_BHD_BC_RECEPTIONIST_X;
			this._np[1] += REM_BHD_BC_RECEPTIONIST_Y;	
		}
		else if(Karryn.isInOrcPaizuriSexPose()) {
			this._np[0] += REM_BHD_BC_ORC_PAIZURI_X;
			this._np[1] += REM_BHD_BC_ORC_PAIZURI_Y;	
		}
		else if(Karryn.isInGoblinCunnilingusSexPose()) {
			this._np[0] += REM_BHD_BC_GOBLIN_CL_X;
			this._np[1] += REM_BHD_BC_GOBLIN_CL_Y;	
		}
		else if(Karryn.isInReverseCowgirlPose()) {
			this._np[0] += REM_BHD_BC_REVERSE_COWGIRL_X;
			this._np[1] += REM_BHD_BC_REVERSE_COWGIRL_Y;	
		}
		else if(Karryn.isInKarrynCowgirlPose()) {
			this._np[0] += REM_BHD_BC_KARRYN_COWGIRL_X;
			this._np[1] += REM_BHD_BC_KARRYN_COWGIRL_Y;	
		}
		else if(Karryn.isInLizardmanCowgirlPose()) {
    	...
    	this._np[0] = (this.width / 2) + this._wh[0] + REM_BHD_BC_COM_X + this._side[i][0];
	    this._np[1] = (this.height / 2) + this._wh[1] + REM_BHD_BC_COM_Y + this._side[i][1];
		if(Karryn.isInUnarmedPose()) {
			this._np[0] += REM_BHD_BC_UNARMED_X;
			this._np[1] += REM_BHD_BC_UNARMED_Y;	
		}			
...
Scene_Battle.prototype.create_layout_window = function() {
	...
	if (this._party_layout) {this.removeChild(this._party_layout)};
